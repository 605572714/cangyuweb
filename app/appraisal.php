	...
	<div ref="vue" id="vue" class="vue">
		...
		<div class="detail">
			...
			<div id="album" ref="album" class="album">
				<img class="img" :style="checkSize(detail.album.length)" v-for="item,index in detail.album" :src="checkImg(item.file_path,'img')" @click="bigImg(item.file_path)">
			</div>
			<!-- 掌眼 -->
			<div class="palm" v-if="detail.palm!=''">
				<span class="title" v-text="'邀请掌眼：'"></span>
				<span class="nickname" v-for="item,index in detail.palm" v-text="item.nickname"></span>
			</div>
		</div>
		<!-- 分隔行 -->
		<div class="hr"></div>
		...
		<div class="more-comment" v-if="total>list.length" v-text="'更多评论内容请前往APP查看'"></div>
		<div class="comment-finish" v-if="total==list.length&&total!=0" v-text="'无更多评论'"></div>
		<!-- Bottom Tabbar-->
...
<script>
	...
	var app = new Vue({
		...
		methods: {
			...
			getDetail() {
				var that = this;
				axios.get(`${CYHOST}/icyApi/details_identify`, {
					params: {
						id: square_id
					}
				}).then(res => {
					...
					console.log('鉴定详情', that.detail);
					window.share_config = {
						"share": {
							"imgUrl": that.detail.album[0].file_path || "http://www.icangyu.com/cangyuxyidong/img/icon.png",
			...
			checkSize(size) {
				switch (size) {
					case 1:
						return 'width:60vw;height:60vw';
						break;
					case 4:
						return 'width:40vw;height:40vw';
						break;
					default:
						return '';
						break;
