...
<body>
    <div ref="vue" id="vue" class="vue">
        <video v-if="detail.video_url&&detail.video_url!=0" :poster="detail.video_cover" controls>
        ...
        <div class="status" :style="checkStatus(detail,'background')">
          ...
          timeData.minutes }分${ timeData.seconds }秒`">
                    ...
                    <span class="time" v-if="checkStatus(detail,'text')=='已结束'||checkStatus(detail,'text')=='已结缘'" v-text="detail.time">
                    </span>
                </template>
            </van-count-down>
        </div>
        <div class="top">
            ...
            <div class="price">
                <span class="now-price" v-text="`￥${detail.now_price}`"></span>
                <span class="old-price" v-text="`原价 ${detail.old_price}`"></span>
            </div>
        </div>
        ...
        <div class="detail">
            ...
            <div class="my-gallery album" data-pswp-uid="1">
                <figure v-for="item,index in detail.album">
                    <div>
                        <a :href="checkImg(item.file_path)" data-size="1920x1080">
                            <img :src="checkImg(item.file_path)">
                        </a>
                    </div>
                </figure>
            </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            getDetail() {
                var that = this;
                axios.get(`${CYHOST}/icyApi/spike_details?id=${square_id}`).then(res => {
            ...
            checkStatus(item, type) {
                switch (item.status) {
                    case 1:
                        ...
                        break;
                    case 2:
                        return type == 'text' ? '进行中' : 'background:#ffe7d4';
                        break;
                    case 3:
                        if (item.nums == 0) {
                            return type == 'text' ? '已结缘' : 'background:lightgray';
                        } else {
            ...
            goApp() {
                var configs = [{
                    ...
                    params: {
                        'shareID': square_id,
                        'shareIOS': 'KCSecKillDetailVC',
                        'shareAndroid': 'icangyu.jade.activities.seckill.SecKillDetailsActivity'
                    }
                }];
                new JMLink(configs);
            },
