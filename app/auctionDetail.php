...
<body>
    <div ref="vue" id="vue" class="vue">
        ...
        <table>
            <tr>
                <td v-for="item in detail.price" style="color: #888" v-text="item.title"></td>
            </tr>
            <tr>
                <th v-for="item in detail.price" style="color: #bc2e2e" v-text="`${item.value}`"></th>
            </tr>
        </table>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            getDetail() {
                var that = this;
                axios.get(`${CYHOST}/icyApi/sale_details?id=${square_id}`).then(res => {
            ...
            goApp() {
                var configs = [{
                    ...
                    params: {
                        'shareID': square_id,
                        'shareIOS': 'KCAuctionMessVC',
                        'shareAndroid': 'icangyu.jade.activities.auction.AuctionDetailsActivity'
                    }
                }];
                new JMLink(configs);
            },
