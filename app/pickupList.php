...
<body>
    <div ref=vue id="vue" class="vue">
        <div class="top">
            <div>
                <van-count-down class="time" :style="header.status==3?'color:#fff':''" :time="header.timeStamp" :format="checkTime(header,'text')" @finish="getDetail" />
            </div>
        ...
        <div class="rule">
            <span v-text="'点击查看捡漏规则'" @click="ruleDetail"></span>
        </div>
        <div class="list">
            <div class="item" v-for="item,index in list" @click="spickDetail(item.id)">
                ...
                <div class="price">
                    ...
                    <span class="value" v-text="`￥${item.link_price}`"></span>
                </div>
                <span class="check" :style="item.status==3?'background:#cbcbcb;':'background:#bc2e2e;'" v-text="item.status==3||item.bid_number==0?'已结束':'去看看'"></span>
            </div>
        </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            getDetail() {
                var that = this;
                axios.get(`${CYHOST}/icyApi/pick_up_list?id=${square_id}`).then(res => {
            ...
            checkTime(item, type) {
                ...
                switch (item.status) {
                    case 1:
                        return type == 'text' ? `即将开始 距开始还剩 DD 天 HH 时 mm 分 ss 秒` : 'img/header_fen.png';
                        break;
                    case 2:
                        return type == 'text' ? `进行中 距结束还剩 DD 天 HH 时 mm 分 ss 秒` : 'img/header_huang.png';
                        break;
                    case 3:
            ...
            // 跳转规则页面
            ruleDetail() {
                window.location.href = 'appRuleDetail.html?item_id=rule005';
            },
            // 跳转详情
            spickDetail(id) {
                window.location.href = `pickupDetail.php?item_id=${id}`;
            },
            // 跳转APP
            goApp() {
                var configs = [{
                    ...
                    params: {
                        'shareID': square_id,
                        'shareIOS': 'KCPickupListVC',
                        'shareAndroid': 'icangyu.jade.activities.pick.PickSessionActivity'
                    }
                }];
                new JMLink(configs);
            },
