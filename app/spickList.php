...
<head>
    ...
    <link rel="stylesheet" href="css/spickList.css">
...
<body>
    <div ref=vue id="vue" class="vue">
        <div class="top">
            <div>
                <van-count-down class="time" :time="header.timeStamp" :format="checkTime(header,'text')" @finish="getDetail" />
            </div>
        ...
        <div class="rule">
            <span v-text="'点击查看秒杀规则'" @click="ruleDetail"></span>
        </div>
        <div class="list">
            <div class="item" v-for="item,index in list" @click="spickDetail(item.id)">
                ...
                <div class="old-price">
                    <span class="title" v-text="'原价'"></span>
                    <span class="value" v-text="`￥${item.old_price}`"></span>
                </div>
                <div class="price">
                    <span class="title" v-text="'秒杀价'"></span>
...
<script>
    ...
    var vue = new Vue({
        ...
        methods: {
            getDetail() {
                var that = this;
                axios.get(`${CYHOST}/icyApi/spike_list?id=${square_id}`).then(res => {
            ...
            // 跳转规则页面
            ruleDetail() {
                window.location.href = 'appRuleDetail.html?item_id=rule012';
            },
            // 跳转详情
            spickDetail(id) {
                window.location.href = `spickDetail.html?item_id=${id}`;
            },
            // 跳转支持
            support() {
                this.$dialog.alert({
