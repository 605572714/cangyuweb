...
<body>
    ...
    <script type="text/html" id="car_mess_div_list2">
        <ul>
            ...
            <div class="card_yanxuan">
                <a href="pickupContent.php?item_id=<%=list[i].id%>" class="block-a">
                    <div class="card_zhuanti demo-card-header-pic">
                        <%if(list[i].pic_url.indexOf('http')==-1){%>
                        <img src="https://app.icangyu.com<%=list[i].pic_url%>" style='width:100%;' alt="">
                        <%}else{%>
                        <img src="<%=list[i].pic_url%>" style='width:100%;' alt="">
                        <%}%>
                    </div>
    ...
    <script>
        ...
        //拍品列表
        $.getJSON(`https://app.icangyu.com/icyApi/pick_up_list?screenings=${square_id}`, function(data) {
            console.log(data);
            var Odata = data.data;
            ...
            window.share_config = {
                "share": {
                    ...
                    "desc": Odata.header.content, //摘要,如果分享到朋友圈的话，不显示摘要。
                    "title": Odata.header.screenings_des, //分享卡片标题
