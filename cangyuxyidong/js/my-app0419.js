...
$.getJSON(`${CYHOST}/icy/details_product?id=0&token=${token}`, function (data) {
    // console.log(data);
    var Odata = data.data;
    var html = template('product_mess_list', Odata);
    $('.product_mess_div_list').html(html);
})

// 专题列表
$.getJSON(`${CYHOST}/icy/carousel_list?page=0`, function (data) {
    // console.log(data);
    var Odata = data.data;
    var html = template('car_list', Odata);
    $('.car_div_list').html(html);
})
...
$.getJSON('https://app.icangyu.com/icy/topic_recommend', function (data) {
    ...
    // console.log(urlObj)
    // sessionStorage.array = JSON.stringify(urlObj);
    store.set('topic_array', urlObj);
})
...
//首页关注
$.getJSON(`${CYHOST}/icy/attention_dynamic?page=0&token=${token}`, function (data) {
    // console.log(data);
    var Odata = data.data;
